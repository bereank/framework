...
class GPMBackUpModeApiController extends Controller
{
    /** GET ALL DOCUMENTS */
    ...
    public function store(Request $request)
    {
        ...
        if (!$record) {
            ...
            if ($isBackupMode) {
                ...
                if (BackUpModeEntries::where('DocNum', $fullDocNum[2])->where('ObjType', $ObjType)->where('ReleaseStatus', 1)->exists()) {
                    return response()
                        ->json(
                            [
                    ...
                    $scanLogData = BackUpModeEntries::firstOrNew(
                        [
                            'DocNum' => $fullDocNum[2],
                            'ObjType' => $ObjType,
                        ],
                    ...
                    return response()
                        ->json(
                            [
                                ...
                                'BackUpMode' => 1,
                                'type' => '',
                                'ScanLogId' => $scanLogData->id,
                                'resultDesc' => 'Back up mode on: Kindly confirm the Document',
                                'Details' => [
                                    "DocumentDetails" => "Ok: Back up mode on"
                                ],
                            ],
                            200
                        );
    ...
    public function update(Request $request, $id)
    {
        ...
        try {
            if ($request['BackUpMode'] == 1) {
                $data = BackUpModeEntries::findOrFail($id);
